...
public class PostCommentsManager implements PostCommentsManagerInterface{
	...
	public void submitPostComment(String text, String userId, String masterPostId, String postIndex) throws
		...
		String query = "insert into post_comments (text, user_id, masterPost_id, postIndex) values ('" + text + "', '"
				+ userId + "', '" + masterPostId + "', '" + postIndex + "');";
	...
	private ArrayList<Post> getPostsFromResultSet(ResultSet rs) throws NumberFormatException, SQLException,
			ParseException{
		...
		while(rs.next()){
			...
			posts.add(new Post(id, text, likes, dislikes, total, timestamp, timeString, dateString, canRate,
					postUser));
		}
